...
| `lens.mjs` | a realidade aumentada pela câmera: a **foto de uma mão** vira a câmera do Chromium (um MJPEG da própria foto), a página é servida com os **cabeçalhos reais** do servidor e o modelo roda de verdade — os cinco dedos desenhados cada um na sua cor, "mão aberta" lida, um holograma por voz sobre a câmera, **o sintetizador tocando uma nota com som medido na saída de verdade e sem chegar ao teto**, a esfera arrastada até a palma da foto e **pousando nela**, a câmera desligada ao sair. Também sem conseguir baixar o modelo, e com o cabeçalho antigo: aí o app manda fazer o deploy, não conferir a conexão. Precisa do pacote npm e do modelo locais (ver o cabeçalho do arquivo) |
...
| `fluency.mjs` | quanto tempo até ele começar a falar: um modelo que transmite devagar (1,2 s até a primeira palavra) e uma voz que anota a hora de cada fala. Mede antes/depois (com uma segunda porta servindo a versão anterior): 3,45 s de silêncio → "Hum…" em 0,65 s e a primeira frase em 1,39 s. E, com a fala do arquivo no microfone, se ele diz "aham" nas pausas curtas sem repetir |
